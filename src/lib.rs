...
//! native gate set) -> [`optimize::optimize`] (native-level peephole
//! cleanup) -> [`fidelity::estimate_circuit_fidelity`] (quick
//! sanity-check number) -> [`emit::run`] / [`emit::run_backend`]
//! (actually execute it on `sirraya_qutub`).
//!
