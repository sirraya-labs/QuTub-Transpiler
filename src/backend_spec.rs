...
//! (`backend_trapped_ion.rs`, `backend_ibmq.rs`, `backend_rigetti.rs`),
...
impl Backend {
    ...
    pub const TrappedIon: Backend = Backend(&crate::backend_trapped_ion::TrappedIonSpec);
    /// IBM-superconducting-style target: `{Rz, Rx, Cx}`.
    #[allow(non_upper_case_globals)]
    pub const IbmQ: Backend = Backend(&crate::backend_ibmq::IbmQSpec);
    /// Rigetti-superconducting-style target: `{Rz, Rx, Cz}`.
    #[allow(non_upper_case_globals)]
    pub const Rigetti: Backend = Backend(&crate::backend_rigetti::RigettiSpec);
